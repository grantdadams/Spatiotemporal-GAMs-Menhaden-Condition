...
wind.bam.vc.lag.2=bam(relative.condition~ s(longitude,latitude) + s(latitude , longitude, by = monthly.u.wind.lag2) + s( latitude , longitude, by = monthly.v.wind.lag2) + s(month, k = 6) + factor(year) + s(vessel, bs = "re"), data= df.menhaden.short, method = "ML", family = Gamma(link = "log"))
model <- wind.bam.vc.lag.2
...
pred.UWind.vec=pred[[1]][,3]
sigCI <- ifelse(pred.UWind.vec+1.96*pred$se.fit[,3]>=0 & pred.UWind.vec-1.96*pred$se.fit[,3]<0,0,1) #Is the CI significantly different from zero? 0 if no, 1 if yes
...
# V- Wind Coefficients
######################
pred.VWind.vec=pred[[1]][,4]
sigCI.v <- ifelse(pred.VWind.vec+1.96*pred$se.fit[,4]>=0 & pred.VWind.vec-1.96*pred$se.fit[,4]<0,0,1) #Is the CI significantly different from zero? 0 if no, 1 if yes
...
######## Plot it
######################
tiff(file="GAMM_Wind_Plot_Short.tiff" , height= 110,  width=160 , pointsize=10 , family = "serif" , units = "mm", res =300)
...
# No Cluster
###################
tiff(file="GAMM_Wind_Plot_No_Cluster.tiff" , height= 110,  width=160 , pointsize=10 , family = "serif" , units = "mm", res =300)
nf <- layout(matrix(c(1,1,2,2,3,4,5,5) , 4 , 2 , byrow=TRUE) ,  heights=c(1 , 1 , 1 , .4) , widths = c(2.77,1),  TRUE)
par( mar=c(0, 3 , .5 , 4) , oma=c(0 , 1 , 0 , 0))
...
#points(y = df.menhaden.short$latitude, x=df.menhaden.short$longitude)
box(which = "plot", lty = "solid")
legend("topleft", "a. Zonal wind vector (E-W)", bg = "grey90", cex=1.5) 
...
#points(y = df.menhaden.short$latitude, x=df.menhaden.short$longitude)
box(which = "plot", lty = "solid")
legend("topleft", "b. Meridional wind vector (N-S)", bg = "grey90", cex=1.5)
