...
model <- bam(residuals ~ s(SST_9km_lag_0) + s(latitude, longitude, by = chl_9km_lag_2), data= df.menhaden.short, method = "fREML")
...
pred.vec=pred[[1]][,2]
sigCI <- ifelse(pred.vec+1.96*pred$se.fit[,2]>=0 & pred.vec-1.96*pred$se.fit[,2]<0,0,1) #Is the CI significantly different from zero? 0 if no, 1 if yes
