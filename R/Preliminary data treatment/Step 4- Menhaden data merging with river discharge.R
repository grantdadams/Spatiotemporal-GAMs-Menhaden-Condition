...
# data.frame.menhaden <- read.csv("Data\\NMFS Menhaden Surveys 1964-2011 mid-treatment\\NMFS Menhaden survey data set 1964-2011 with location step 3.csv")
data.frame.menhaden$date<- as.Date(data.frame.menhaden$date, origin="1970-01-01")
...
data.frame.menhaden=merge(data.frame.menhaden,MISS.ATCH.river.data, by=c("day","month","year"), all.x=T) #merging the daily data
data.frame.menhaden=merge(data.frame.menhaden,monthly.discharge,by=c("year","month"))
data.frame.menhaden=merge(data.frame.menhaden,March.to.May.discharge,by=c("year"))
data.frame.menhaden<- merge(data.frame.menhaden, Nov.to.March.discharge, by = c("year"))

# write.csv(data.frame.menhaden, file = "Data\\NMFS Menhaden Surveys 1964-2011 mid-treatment\\NMFS Menhaden survey data set 1964-2011 with location step 4.csv")
