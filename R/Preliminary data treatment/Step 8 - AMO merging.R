...
amo_data <- read.csv("Data\\Atlantic multi-decadal oscillation.csv")
library(reshape)
...
nao_data <- read.csv("Data\\Monthly_mean_north_atlantic_oscillation_index.csv")
library(reshape)
...
# - Merge
# data.frame.menhaden <- read.csv("Data\\NMFS Menhaden survey data set 1964-2011 with location step 7")

data.frame.menhaden= merge(data.frame.menhaden, amo_data, by=c("month","year"))
data.frame.menhaden= merge(data.frame.menhaden, nao_data, by=c("month","year"))
