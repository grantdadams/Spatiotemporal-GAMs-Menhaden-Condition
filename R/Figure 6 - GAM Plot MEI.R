...
#tiff(file="Figures/Fig_6_GAM_ENSO_Plot.tiff" , height=2.9 ,  width=3.34646 , pointsize=10 , family = "serif" , units = "in", res =600)
postscript("Figures/Fig_6_GAM_ENSO_Plot.eps", horizontal = FALSE, onefile = FALSE, paper = "special", height=2.9 ,  width=3.34646, pointsize = 10, family = "serif" )
par(xpd=F)
nf <- layout(matrix(c(1:2) , 2 , 1 , byrow=TRUE) ,  heights=c(1 , .6 ) ,  TRUE)
par( mar=c(0 , 2 , 1 , 1) , tcl=-.25 , mgp=c(2 ,  .5 ,  0) ,  oma=c(0 , 2 , 0 , 0))
plot(model , scale=0 , pch=19 , scheme=1 , col=1 , shade.col='gray90' ,  ylab=NA , select=3 , xlim=c(min(df.menhaden$mei.lag.10, na.rm=T) , max(df.menhaden$mei.lag.10, na.rm=T)), xlab = NA, rug =F, xaxt="n", las =1)
legend("topleft", "a.", bty="n", inset = -0.05)
title(ylab=substitute(paste("Effect on ", italic("Kn" ))), line=-0.3 , outer=T , adj = .7)
...
title(ylab="(x 10,000)", line=-.80 , outer=T , adj = .21)
